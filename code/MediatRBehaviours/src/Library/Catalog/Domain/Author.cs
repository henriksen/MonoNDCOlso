...
public class Author
{
    ...
    public List<Guid> Books { get; private set; } = [];
...
#pragma warning restore CS8618 // Non-nullable field must contain a non-null value when exiting constructor. Consider declaring as nullable.
    ...
    public static Result<Author> Create(AuthorId id, string name, string biography, DateTimeOffset? birthDate, DateTimeOffset? deathDate)
    {
        if (birthDate >= deathDate)
        {
            return Result.Fail("Birth date must be before death date.");
        }
        return Result.Ok(new Author(id, name, biography, birthDate, deathDate));
    }
